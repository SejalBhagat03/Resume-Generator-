...
def generate_pdf(data=None, pdf_filename=None, accent_color="#2563EB", font_scale=1.0, margin_size=20):
    ...
    # Dynamically determine the PDF filename if not provided
    ...
    email_text = f"<a href='mailto:{email}'><font color='{accent_color}'>{email}</font></a>" if email else ""
    ...
    linkedin_text = f"<a href='{linkedin_url}'><font color='{accent_color}'>{linkedin_display}</font></a>" if linkedin_url else ""
    right_p1 = Paragraph(linkedin_text, right_align_style)
    
    github = personal.get("github", {})
    github_url = github.get("url", "")
    github_display = github.get("display", "")
    github_text = f"<a href='{github_url}'><font color='{accent_color}'>{github_display}</font></a>" if github_url else ""
